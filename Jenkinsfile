            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 192.168.88.222:8085/config-server:${VERSION} .
                                docker login -u admin -p $docker_password 192.168.88.222:8085
                                docker push  192.168.88.222:8085/config-server:${VERSION}
                                docker rmi 192.168.88.222:8085/config-server:${VERSION}
                            '''
                    }
                }
            }
